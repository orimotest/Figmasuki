...
設定ドロワーの制作モードで `Demo` を選ぶと、外部APIを呼ばずに制作フローを確認できます。

1. プラグインを起動
2. 要件入力で「セミナーサンプル」を読み込む
3. 「この要件で自動制作へ」から自動制作を開始
...
設定ドロワーで制作モードを `API` に切り替え、Dify / Geminiの接続情報を保存するとAPI接続を使えます。`Demo` のままなら、接続情報が入っていても外部APIは呼びません。
...
- [docs/markdown-requirement-input.md](docs/markdown-requirement-input.md)
- [docs/figma-auto-layout-output.md](docs/figma-auto-layout-output.md)
